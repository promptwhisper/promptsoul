...
function errorResponse(error: unknown): Response {
  if (error instanceof VoiceConfigurationError || error instanceof LocalMutationError) {
...
export async function GET(): Promise<Response> {
  try {
    return json(getPublicVoiceSettings());
  } catch (error) {
...
export async function POST(request: Request): Promise<Response> {
  try {
    ...
    return json(setRuntimeVoiceSettings(await readJsonMutation(request)));
  } catch (error) {
    return errorResponse(error);
  }
}
...
export async function DELETE(request: Request): Promise<Response> {
  try {
    assertLocalSameOriginMutation(request);
    return json(resetRuntimeVoiceSettings());
  } catch (error) {
    return errorResponse(error);
  }
}
