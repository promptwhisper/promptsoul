...
- `lib/server/voice-store.ts`, `lib/server/voice-service.ts`: independent process-memory TTS settings and the AITuber Voice adapter.
...
The voice API accepts only `{text, emotion}` and returns audio bytes. TTS configuration is independent from chat configuration; voice failure must never break text chat or motion selection. Lip sync may update runtime mouth parameters only and must never persist model or motion changes.
...
- Chat and voice configuration mutations must remain loopback-only, same-origin JSON requests. Do not expose this development server directly to the public internet.
